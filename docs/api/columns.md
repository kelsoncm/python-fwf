...
Right-aligned text field. Behaviour is identical to `CharColumn` on read (spaces stripped); on write, the value is right-aligned within the field width.
...
| `_format` | `str` | `'%d%m%Y%H%M'` | `strftime`-compatible format (must contain exactly 5 format codes) |
